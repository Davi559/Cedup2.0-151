...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($subject_id) || empty($title) || empty($content_type)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO content (subject_id, user_id, title, content_type, date, content_text) VALUES (?, ?, ?, ?, ?, ?)");
            ...
            if ($content_type === 'image' || $content_type === 'pdf') {
                if ($file && $file['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($file['tmp_name'], $file_path)) {
                        $stmt = $conn->prepare("UPDATE content SET file_path = ? WHERE id = ?");
