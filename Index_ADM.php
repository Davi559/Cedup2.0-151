<?php
session_start();
require_once 'config/database.php';


if (!isset($_SESSION['user_id'])) {
    header('Location: LoginADM.php');
    exit;
}




// Ações: ban, unban, promote
if (isset($_GET['action']) && isset($_GET['user_id'])) {
    $userId = (int)$_GET['user_id'];

    if ($_GET['action'] === 'ban') {
        $stmt = $conn->prepare("UPDATE users SET is_banned = 1 WHERE id = ?");
        $stmt->execute([$userId]);
    } elseif ($_GET['action'] === 'unban') {
        $stmt = $conn->prepare("UPDATE users SET is_banned = 0 WHERE id = ?");
        $stmt->execute([$userId]);
    } elseif ($_GET['action'] === 'promote') {
        $stmt = $conn->prepare("UPDATE users SET is_admin = 1 WHERE id = ?");
        $stmt->execute([$userId]);
    }

    header("Location: Index_ADM.php");
    exit;
}

// Buscar matérias
$stmt = $conn->query("SELECT * FROM subjects ORDER BY name");
$subjects = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Buscar usuários
$stmtUsers = $conn->query("SELECT id, username FROM users");
$users = $stmtUsers->fetchAll(PDO::FETCH_ASSOC);
?>

<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <title>Admin - Cedup</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/style.css">
</head>
<body>
    <nav class="navbar navbar-expand-lg navbar-dark">
        <div class="container">
            <a class="navbar-brand" href="index.php">
                <img src="assets/img/cedup-logo.png" alt="Cedup Logo">
                Cedup
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
                <span class="navbar-toggler-icon"></span>
            </button>
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    <li class="nav-item">
                        <a class="nav-link active" href="index.php">Início</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="upload.php">Compartilhar Conteúdo</a>
                    </li>
                </ul>
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <span class="nav-link">Olá, <?php echo htmlspecialchars($_SESSION['username']); ?></span>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="logout.php">Sair</a>
                    </li>
                </ul>
            </div>
        </div>
    </nav>
    <div class="container mt-4">
        <h1 class="text-center mb-4">Matérias Disponíveis</h1>
        <div class="row">
            <?php foreach ($subjects as $subject): ?>
                <div class="col-md-4 mb-4">
                    <div class="card h-100">
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($subject['name']) ?></h5>
                            <p class="card-text"><?= htmlspecialchars($subject['description']) ?></p>
                            <a href="../subject.php?id=<?= $subject['id'] ?>" class="btn btn-primary">Ver Conteúdo</a>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
            </div>
            <tbody>
                <?php foreach ($users as $user): ?>
                            <?php if (!$user['is_admin']): ?>
                                <a href="?action=promote&user_id=<?= $user['id'] ?>" class="btn btn-warning btn-sm">Promover a ADM</a>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    <meta charset="UTF-8">
    <title>Meu Site</title>
    <link rel="stylesheet" href="styles.css">
</head>
<body>
    <?php include 'menu.php'; ?>

    <div class="conteudo-principal" style="margin-left: 200px;">
    </div>

    <script>
        function toggleMenu() {
            const menu = document.getElementById("menuVertical");
            menu.classList.toggle("minimizado");
            const conteudo = document.querySelector(".conteudo-principal");
            if (menu.classList.contains("minimizado")) {
                conteudo.style.marginLeft = "20px";
            } else {
                conteudo.style.marginLeft = "200px";
            }
        }
    </script>
</body>
</html>
